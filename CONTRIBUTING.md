# Contributing to OnesLLM

OnesLLM is an open-source project and we welcome contributions from the community.
...
1. If you are modifying the permission system for a new role or something custom, you are likely better off forking the project and building your own version since this is a core part of the project and is only to be maintained by the OnesLLM team.
...
The core library is written in Node.js. There are additional sub-repositories for the embed widget and browser extension. These are not part of the core OnesLLM project, but are maintained by the OnesLLM team.
...
Changes to the core OnesLLM project are released through the `master` branch. When a PR is merged into `master`, a new version of the package is published to Docker and GitHub Container Registry under the `latest` tag.
...
Changes to the desktop app are downstream of the core OnesLLM project. Releases of the desktop app are published at the same time as the core OnesLLM project. Code from the core OnesLLM project is copied into the desktop app into an Electron wrapper. The Electron wrapper that wraps around the core OnesLLM project is **not** part of the core OnesLLM project, but is maintained by the OnesLLM team.

## License

By contributing to OnesLLM (this repository), you agree to license your contributions under the MIT license.
